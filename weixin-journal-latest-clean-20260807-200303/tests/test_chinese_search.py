...
def test_resolve_keyword_plan_uses_dictionary_for_terminal_transferase():
    called_model = False
    ...
    plan = resolve_keyword_plan(["核酸末端转移酶"], resolver=resolver)

    assert called_model is False
    assert plan.keywords[0].source == "dictionary"
    assert "terminal deoxynucleotidyl transferase" in plan.keywords[0].english_terms
    assert "template-independent DNA synthesis" in plan.keywords[0].english_terms
...
def test_federated_search_default_sources_exclude_openalex_without_api_key(monkeypatch):
    ...
    def fail_openalex(*args, **kwargs):
        raise AssertionError("OpenAlex should not run without an API key by default")

    monkeypatch.setattr("weixin_lite.search.search_pubmed", empty_source)
    monkeypatch.setattr("weixin_lite.search.search_europe_pmc", empty_source)
    monkeypatch.setattr("weixin_lite.search.search_crossref_many", empty_source)
    monkeypatch.setattr("weixin_lite.search.search_openalex", fail_openalex)

    records, errors = federated_search(["核酸末端转移酶"], limit=5, openalex_api_key="")
